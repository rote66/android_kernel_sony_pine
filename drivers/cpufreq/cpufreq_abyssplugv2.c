...
static inline u64 get_cpu_idle_masa(unsigned int cpu, u64 *wall)
{
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		struct cpu_bds_info_s *bds_info;
		bds_info = &per_cpu(od_cpu_bds_info, j);
		bds_info->prev_cpu_idle = get_cpu_idle_masa(j,
...
static void bds_refresh_callback(struct work_struct *unused)
{
	...
	if (policy->cur < policy->max) {
		...
		__cpufreq_driver_target(policy, policy->max,
					CPUFREQ_RELATION_L);
		this_bds_info->prev_cpu_idle = get_cpu_idle_masa(cpu,
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_bds_info->prev_cpu_idle = get_cpu_idle_masa(j,
