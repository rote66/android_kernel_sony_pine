...
int FST_AK8963(void)
{
	...
	/* return 0; */
	/* } */
	if (AKECS_Reset(0) < 0) {	//fix coverity issues, 161208
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return 0;
	}
	mdelay(1);
...
static int akm09918_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strcpy(info->type, AKM09918_DEV_NAME);
	return 0;
}
...
static int akm09918_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->hw = hw;
	data->hw->direction = 8;	//change direction for SM31
	...
	printk("[akm09918] kernel: auto probe address 0x0c\n");
